...
namespace AmplifyImpostors
{
	...
	{
		bool m_copyMaterialProperties = false;
		...
		private static readonly GUIContent BakingPresetExtStr = new GUIContent( "Bake Preset (Extended)", "Preset object that contains the baking configuration. When empty it will use the standard preset." );
		private static readonly GUIContent AdvancedStr = new GUIContent( "Advanced", "Advanced settings for power users. Use only if you know what you're doing." );
		...
		public void OnEnable()
		{
			...
			if( m_currentData.Preset == null )
			{
				...
				m_currentData.Preset = AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>( AssetDatabase.GUIDToAssetPath( Preferences.GlobalBakePresetGUID ) );

				if( m_currentData.Version < 8500 )
				{
					int outputCount = Mathf.Min( 4, m_currentData.Preset.Output.Count );
		...
		override public void OnInspectorGUI()
		{
			...
			m_currentData.Preset = EditorGUILayout.ObjectField( BakingPresetStr, m_currentData.Preset, typeof( AmplifyImpostorBakePreset ), false ) as AmplifyImpostorBakePreset;
			...
			if( Preferences.GlobalBakingOptions && m_instance.Data != null )
			{
				EditorGUILayout.BeginVertical( "helpbox" );
				{
					...
					EditorGUILayout.BeginHorizontal();
					m_presetOptions = GUILayout.Toggle( m_presetOptions, BakingPresetExtStr, "foldout", GUILayout.Width( EditorGUIUtility.labelWidth - 11 ) );
					EditorGUILayout.BeginVertical();
					EditorGUILayout.Space( -6 );
					EditorGUILayout.BeginHorizontal();
					...
					bool newButtonPressed = GUILayout.Button( "New", "minibutton", GUILayout.Width( 50 ) );
					EditorGUILayout.EndHorizontal();
					EditorGUILayout.EndVertical();
					EditorGUILayout.EndHorizontal();
					if ( newButtonPressed )
					...
					m_copyMaterialProperties = GUILayout.Toggle( m_copyMaterialProperties, "Material Properties", "foldout" );
					if ( m_copyMaterialProperties )
					{
						EditorGUI.indentLevel++;
						m_instance.m_copyPropertiesSource = ( Material )EditorGUILayout.ObjectField( "Copy From", m_instance.m_copyPropertiesSource, typeof( Material ), allowSceneObjects: false );
						m_instance.m_copyPropertiesMask = ( MaterialPropertyCopyMask )EditorGUILayout.EnumFlagsField( "Copy Mask", m_instance.m_copyPropertiesMask );
						EditorGUI.indentLevel--;
					}
					#endregion
					...
					#region ADVANCED
					m_advanced = GUILayout.Toggle( m_advanced, AdvancedStr, "foldout", GUILayout.Width( EditorGUIUtility.labelWidth - 11 ) );
					if ( m_advanced )
					{
						EditorGUI.indentLevel++;
						EditorGUILayout.BeginHorizontal();
						m_instance.m_lastImpostor = ( GameObject )EditorGUILayout.ObjectField( "Impostor Object", m_instance.m_lastImpostor, typeof( GameObject ), true );
						EditorGUILayout.EndHorizontal();
						EditorGUI.indentLevel--;
					}
					#endregion
