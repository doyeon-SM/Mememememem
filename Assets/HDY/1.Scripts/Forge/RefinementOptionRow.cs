...
{
    /// <summary>
    /// 연마 옵션 txt(탭 구분) 1행. 컬럼 순서: 등급\t종류\t수치\t확률
    /// 예: Rare\tDamageIncrease\t1\t50
    ...
    public struct RefinementOptionRow
    {
        ...
        public RefinementOptionRow(CommonClass grade, string optionType, float value, float probability)
        {
            Grade = grade;
    ...
    /// 헤더 행(등급\t종류\t수치\t확률)은 있어도 없어도 무방하게 처리한다.
    /// </summary>
    public static class RefinementOptionCsvParser
    {
        ...
        public static List<RefinementOptionRow> Parse(string text)
        {
            ...
            foreach (var rawLine in lines)
            {
                ...
                // 헤더 행("등급\t종류\t수치\t확률" 등)은 등급 파싱이 실패하므로 자연히 건너뛰어진다.
                ...
                results.Add(new RefinementOptionRow(grade, optionType, value, probability));
            }
