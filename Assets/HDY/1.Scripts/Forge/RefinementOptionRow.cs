...
{
    /// <summary>
    /// 연마 옵션 csv(쉼표 구분) 1행. 컬럼 순서: 등급,종류,수치,확률,표시명
    ...
    /// <summary>
    /// RefinementOptionRow용 쉼표 구분(CSV) 파서. ItemCatalog.csv와 동일하게 쉼표(',')로 컬럼을 나눈다.
    /// 헤더 행(등급,종류,수치,확률,표시명)은 있어도 없어도 무방하게 처리한다.
    /// </summary>
    public static class RefinementOptionCsvParser
    {
        /// <summary>csv 텍스트 전체를 파싱해 유효한 행만 리스트로 반환한다. 잘못된 행은 건너뛴다.</summary>
